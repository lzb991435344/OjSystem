...
<?php
echo "<center><table class='table table-striped' width=90% border=1>";//<td>Copy
echo "<tr><td>ContestID<td>Title<td>StartTime<td>EndTime<td>Private<td>Status<td>Edit<td>Export<td>Logs";
echo "</tr>";
foreach($result as $row){
        ...
        if(isset($_SESSION[$OJ_NAME.'_'.'administrator'])||isset($_SESSION[$OJ_NAME.'_'."m$cid"])){
                ...
                echo "<td><a href=contest_df_change.php?cid=".$row['contest_id']."&getkey=".$_SESSION[$OJ_NAME.'_'.'getkey'].">".($row['defunct']=="N"?"<span class=green>ON</span>":"<span class=red>OFF</span>")."</a>";
